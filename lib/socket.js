...
import { startMessageWorker } from "../controller/chatController.js";
const app = express();
...
io.on("connection", async (socket) => {
  ...
  const allChannel = [...statusChannel, messageChannel];
  ...
  socket.on("sendMessage", async (payload) => {
    ...
    const { recipientId, content, tempId } = payload;
    ...
    try {
      ...
      socket.on("messageSent", { tempId, status: "send" });
